...
/*   Updated: 2019/11/18 14:38:46 by jacens      ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
							...
							t_bool *struc)
{
	...
	if (format == 'p')
	{
		...
		if (ptr != NULL)
		{
			if (!(str = ft_strjoin(ft_strdup("0x"), ptr))
			|| (struc->arg = ft_strdup("p")) == NULL)
				*sign = -1;
			free(ptr);
		}
	}
	...
	{
		if ((str = ft_strdup(va_arg(args, char *))) == NULL ||
							...
							t_bool *struc)
{
	...
	{
		if (((str = ft_uitoa(va_arg(args, unsigned int))) == NULL)
...
char			*ft_do_flag(va_list args, char format, int *sign, t_bool *struc)
{
	...
	else if (format == 'x')
	{
		...
		== NULL) || (struc->arg = ft_strdup("x")) == NULL)
			*sign = -1;
	}
	else if (format == 'X')
	{
